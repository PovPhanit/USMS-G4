...
namespace University_Student_Management_System.Dashboard.Level
{
    public partial class Level : Form
    {
        ...
        {
            DA = new SqlDataAdapter("select levelID,levelName from level", Operation.con);
            TB = new DataTable();
            DA.Fill(TB);
            LBLevel.DataSource = null;
            LBLevel.Items.Clear();
            LBLevel.DataSource = TB;
            LBLevel.DisplayMember = "levelName";
            LBLevel.ValueMember = "levelID";
        }
        ...
        private void LBLevel_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (!isCreateUPdate)
            {
                txtID.Text = LBLevel.SelectedValue.ToString();
                txtLevel.Text = LBLevel.Text;
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (btnNew.Text == "បោះបង់")
            {
                ...
                if (isCreateUPdate)
                {
                    com = new SqlCommand("insert into level(levelName) values(N'" + txtLevel.Text + "')", Operation.con);
                ...
                {
                    com = new SqlCommand("update level set levelName = N'" + txtLevel.Text + "' where levelID = " + Convert.ToInt32(txtID.Text) + "", Operation.con);
                    int rowEffect = com.ExecuteNonQuery();
                    isLoaded = false;
                    loadData();
                    isLoaded = true;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (re == DialogResult.OK)
            {
                com = new SqlCommand("delete from level where levelID = " + Convert.ToInt32(txtID.Text) + "", Operation.con);
                int rowEffect = com.ExecuteNonQuery();
                isLoaded = false;
                loadData();
                isLoaded = true;
        ...
        {
            if (txtSearch.Text == "Search level hear...")
            {
